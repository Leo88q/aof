...
> **Корректировка от 2026-09-21 (полный аудит, `AUDIT_FULL_2026-09-21.md`).**
> Пометка «Готово» в этой таблице означала «код написан», а не «работает на
> чейне». По факту на момент аудита были **отключены на уровне программы**:
>
> | Фича | Реальный статус | Причина |
> |------|-----------------|---------|
> | Lottery (commit-reveal) | ⛔ reveal отключён | секрет держит authority, нет принудительного settle — держатель секрета выбирает исход (F-06) |
> | Forge (commit-reveal) | ⛔ reveal отключён | плюс бесплатный ретрай через refundable expiry (F-06) |
> | Packs | ⛔ reveal отключён | тот же вектор, что у лотереи (F-06) |
> | Exploration | ⛔ reveal отключён | trip cost сжигается до reveal (F-06) |
> | Reroll (random) | ⛔ commit/reveal отключены | burn происходит до reveal (F-06); детерминированный `reroll` (2→1) работает |
> | Hot Market (VRGDA) | ⛔ торговля отключена | `TradingDisabled` в 3 из 4 инструкций `aof-market` (G-05) |
> | Mining | ⚠️ на тумблере | `Config.mining_enabled`, выключается authority (F-27) |
>
> «Готово» ниже следует читать как «закодировано и закрыто тумблером/включено»
> только для строк без пометки в таблице выше.
...
| Lottery (commit-reveal) | ⛔ reveal | ✅ | ✅ | 🔴 | Отключено (F-06) |
| Hot Market (VRGDA) | ⛔ TradingDisabled | ✅ | ✅ | 🔴 | Отключено (G-05) |
| **Инструменты** | | | | | |
| Mint | ✅ | ✅ | ✅ | — | Готово |
| Stake/Unstake | ✅ | ✅ | ✅ | — | Готово |
| Mining | ✅ (тумблер on-chain) | ✅ | ✅ | — | Готово (F-27) |
| Repair | ✅ (только STONE) | ✅ | ✅ (пульс) | 🟡 | D |
| Craft | ✅ (WOOD+STONE) | ✅ | ✅ (лента) | 🟡 | D |
| Forge (commit-reveal) | ⛔ reveal | ✅ | ✅ | 🔴 | Отключено (F-06) |
| Reroll (детерминированный 2→1) | ✅ | ✅ | ✅ | — | Готово |
| **Экономика** | | | | | |
| GasTank | ✅ | ✅ | ✅ | — | Готово |
| Packs | ⛔ reveal | ✅ | ✅ (анимация) | 🔴 | Отключено (F-06) |
| Exploration | ⛔ reveal | ✅ | ✅ | 🔴 | Отключено (F-06) |
