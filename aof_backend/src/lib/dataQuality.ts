...
export type EconomyFieldQuality = {
  ...
  potatoBurned24h: DataQuality;   // requires on-chain event indexer
  potatoMinted24h: DataQuality;   // requires on-chain event indexer
...
/**
 * Static provenance of the current implementation. Update these as indexers
 * land; the API surfaces them on every snapshot so dashboards can label
 * placeholder values honestly instead of presenting fictitious zeros.
 */
